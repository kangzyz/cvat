...
export default function IssueDialog(props: Props): JSX.Element {
    ...
    const onDeleteIssue = useCallback((): void => {
        ...
        Modal.confirm({
            title: t('review.deleteIssueTitle', { issueNumber }),
        ...
        (_comment: CommentModel): JSX.Element => {
            ...
            return (
                <Comment
                    avatar={null}
                    key={_comment.id}
                    author={<Text strong>{_comment.owner ? _comment.owner.username : t('common.unknown')}</Text>}
    ...
    return ReactDOM.createPortal(
        ...
        >
            <Row className='cvat-issue-dialog-header' justify='space-between'>
                <Col>
                    <Text strong>{typeof id === 'number' ? t('review.issueTitle', { id }) : t('review.issue')}</Text>
                </Col>
                <Col>
                    <CVATTooltip title={t('review.collapseChat')}>

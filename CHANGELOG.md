...
<!--
  Fork notice / Fork 说明：
  本仓库 fork 自上游开源项目 CVAT（https://github.com/cvat-ai/cvat），
  在其基础上进行简体中文本地化与社区版适配改造。

  以下「中文社区版 Fork」小节记录本 fork 自身的版本历史（v0.1.x）；
  其后保留上游 CVAT 原始的版本变更记录（2.x.y）。
-->

## 中文社区版 Fork（基于 https://github.com/cvat-ai/cvat）
...
- 新增企业微信（WeCom）群机器人通知：当 Webhook 目标为企业微信群机器人地址时，自动改用
  Markdown 卡片推送，无需额外配置。
- 企业微信任务/作业通知改版为工作流里程碑消息：采用「项目 / 任务 / 作业」面包屑标题，正文
  首行给出一句话结论（开始、提交、完成、验收、拒绝/退回、进入/退回阶段、重新开启、负责人
  变更、创建、删除），仅展示相关的状态迁移、当前负责人、操作人和前端详情链接，并在任务级
  里程碑附带作业进度快照。
- 质检工作台改版：概览页新增质量结论、风险清单、质量历史、指标卡片、配置检查清单、人工
  问题汇总与证据抽屉，并重做质量设置表单。

#### Changed

- 企业微信通知定位为工作流里程碑推送而非审计流：仅在状态、阶段或负责人等里程碑字段变化时
  发送，纯元数据（如更新时间）变更不再打扰；同一次多字段变更合并为一条消息。
- 通知详情链接指向前端页面（标注/详情页）而非 REST 接口地址，并保留部署的域名、端口与
  路径前缀。
- 保持普通 Webhook 接收方的原生 JSON 请求体、HMAC 签名与投递历史不变；企业微信推送强制
  4096 字节上限、内容转义与应用级 errcode 判定，手动重投递始终发送。

#### Fixed

- 企业微信通知内容与取值全面中文化，事件、状态、阶段、进度等术语与界面一致；未知或新增的
  枚举值原样展示，避免误翻译。
- 企业微信在 HTTP 200 但 errcode 非 0 时正确判定为投递失败并触发重试；连接或超时错误使用
  固定提示文案，避免把含密钥的机器人地址写入投递响应。
...
- 项目、任务和作业详情页新增完整分析能力，统一提供概览、标注和活动三个页签，并支持在
  项目、任务、作业之间逐级下钻。
- 分析概览新增完成度、工作流、帧数、逻辑对象、问题、质量、活跃度和风险指标；标注分析
  新增按标签、类型、来源及帧区间的统计。
- 活动分析新增时间范围、粒度和贡献者筛选，展示工时、对象增删改、审核退回、贡献者及
  工作流变化，并支持查看明细和导出当前资源范围内的 CSV。
- 数据分析的项目详情新增任务原始数据来源，区分上传文件、共享目录或文件、远程地址及
  抽帧数据集，并支持查看多项来源。

#### Changed

- 数据分析入口和接口统一允许 staff 或超级管理员访问，共享视频扫描结果增加短时缓存，
  改善大型共享目录下的响应速度。
- GPU 服务镜像拆分出可复用的运行时基础镜像，并增加 PowerShell 构建与发布脚本，减少
  后续 GPU 镜像重复安装依赖的时间。

#### Fixed

- 修复仅依据 staff 状态判断数据分析权限时，超级管理员入口和接口访问可能不一致的问题。
- 修复任务关联抽帧会话后只显示单一来源的问题；现在会识别同任务使用的其他已保存抽帧
  数据集、保留普通来源，并避免把抽帧数据集的父目录重复显示为普通共享目录。
- 资源活动 CSV 导出会强制限定到当前项目、任务或作业，避免客户端筛选参数扩大导出范围。
...
- 抽帧筛选工作台支持预览已选择的共享视频文件，可在开始抽帧前直接打开浏览器视频控件确认内容。
- 抽帧结果列表新增每页条数选择，支持 30、60、120、240，并在切换后自动回到第一页。
- 已保存的抽帧数据集在未被任务使用时支持撤回保存，恢复为可继续排除/保留图片的待保存状态。
- 新增本地源码挂载 compose 覆盖文件，便于开发环境直接挂载源码测试后端改动。

#### Changed

- 抽帧去重阈值默认值调整为 2，图片质量默认值调整为 80。
- 高级功能占位页改为“功能待开发”提示，并移除“查看价格”按钮。
- 顶部栏移除 GitHub 仓库入口，仅保留本地中文指南入口。

#### Fixed

- 撤回保存会检测抽帧共享目录是否已被任务使用，并在已使用时给出中文提示，避免误删任务数据来源。
- 抽帧共享目录使用检测改为精确匹配目录及其子路径，避免相似前缀目录被误判为已使用。
...
- 数据分析看板（仅管理员）：顶部导航新增「数据分析」入口（仅 staff 可见），提供全局概览、
  按项目下钻的图表分析，以及数据来源/抽帧使用视图；汇总作业完成度、标签与形状类型分布、
  标注来源、共享视频与抽帧用量等实时统计，并将任务关联到其来源的抽帧会话。
- 删除本地 YOLO 模型：「模型」页支持删除本地部署的 YOLO 模型。
- 删除未保存的抽帧会话：抽帧筛选工作台支持删除尚未保存的历史抽帧会话，便于清理。

#### Changed

- 本地 YOLO 部署优化：新增预构建运行时镜像与离线部署选项，并优化部署流程及已部署/待部署
  模型的状态展示。
- 抽帧图片预览优化：改进抽帧筛选工作台的帧预览体验。
...
- 抽帧筛选工作台：从顶部导航进入，可指定共享视频或目录（支持递归），设置抽帧间隔、
  去重阈值、旋转角度与图片质量，预览采样帧并逐帧保留/排除，将筛选后的数据集保存到
  共享目录；历史抽帧任务会保留，可随时查看。

#### Changed

- 抽帧结果一键流转到任务创建：创建任务页自动预载已准备好的共享路径并建议任务名称。
...
- 前端全量简体中文化：页面、弹窗、通知、校验提示、快捷键说明、标注与音频工作区，
  并默认提供中文 Django 管理后台。
- 内置离线中文使用指南，点击应用内帮助（?）按钮即可打开，涵盖安装部署、标注、
  自动标注、数据集管理、快捷键与常见问题。
- 本地 YOLO 模型部署：在「模型」页面上传训练好的 `.pt` 文件，封装为 Nuclio detector
  用于自动标注，无需外部云服务。
- 创建任务时支持视频抽帧去重。
- GPU 加速部署配置（含阿里云 GPU）与 HTTPS/TLS 部署支持。
- 社区版可用的质量控制概览，展示质量设置、最近一次报告摘要、验证帧与冲突。

#### Changed

- 默认关闭公开注册，由管理员后台添加用户（可通过 `CVAT_REGISTRATION_ENABLED=true`
  重新开放）。
- 仓库、更新日志、许可证与指南链接改为指向社区 fork 与内置本地指南，不再依赖外部商业站点。
- 优化文档页面：卡片式布局、侧栏导航、上一篇/下一篇翻页与更佳的 Markdown 排版。

#### Fixed

- 修复标注界面绘制/标签弹出框的快捷键提示回退问题。
- 确保 Docker entrypoint 使用 LF 换行，使容器在 Windows 检出环境下正常启动。

---

## 上游 CVAT 变更记录（Upstream CVAT changelog）
